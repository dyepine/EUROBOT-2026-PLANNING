...
class DepositPoint:
    ...
    def allows_deposit_for_side(self, side: Side) -> bool:
        if self.owner is not None and self.owner is not side:
            return False
        if self.protected_for is not None and self.protected_for is not side:
            return False
        return True
...
class Thermometer:
    ...
    doing_blue: bool = False
    doing_yellow: bool = False
    ...
    def is_doing_for_side(self, side: Side) -> bool:
        return self.doing_blue if side is Side.BLUE else self.doing_yellow

    def mark_doing_for_side(self, side: Side, doing: bool = True) -> None:
        if side is Side.BLUE:
            self.doing_blue = doing
            return
        self.doing_yellow = doing

    def clear_doing_for_side(self, side: Side) -> None:
        self.mark_doing_for_side(side, doing=False)

    def mark_done_for_side(self, side: Side) -> None:
        self.clear_doing_for_side(side)
        if side is Side.BLUE:
...
class Robot:
    ...
    side: Side
    position: Vec2
    speed: float = DEFAULT_ROBOT_SPEED_MPS
    load: int = 0
    capacity: int = 4
...
@dataclass(slots=True, frozen=True)
class Mars:
    # Simplified SIMA/PAMI model used for endgame scoring and visualization.
    name: str
    side: Side
    pantry_id: int
    start_position: Vec2
    target_position: Vec2
    release_time: float = 85.0
    ...
    def position_at(self, t: float) -> Vec2:
        ...
        duration = max(self.arrival_time - self.release_time, 1e-6)
        progress = (t - self.release_time) / duration
        return (
            self.start_position[0] + (self.target_position[0] - self.start_position[0]) * progress,
            self.start_position[1] + (self.target_position[1] - self.start_position[1]) * progress,
        )

    def is_released(self, t: float) -> bool:
        return t >= self.release_time

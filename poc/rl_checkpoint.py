...
def normalize_torch_rng_state(state: object) -> object:
    ...
    if isinstance(state, torch.Tensor):
        tensor = state.detach().to(device="cpu", dtype=torch.uint8).contiguous().view(-1)
        return tensor if isinstance(tensor, torch.ByteTensor) else torch.ByteTensor(tensor.tolist())
    if isinstance(state, (bytes, bytearray, memoryview)):
        return torch.ByteTensor(list(bytes(state)))
    return torch.ByteTensor(state)
...
def normalize_torch_cuda_rng_state_all(states: object) -> list[object] | None:
    ...
    if not isinstance(states, list):
        raise TypeError("CUDA RNG state collection must be a list or tuple of byte tensors")
    return [normalize_torch_rng_state(state) for state in states]

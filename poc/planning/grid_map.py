...
from poc.domain.entities import DepositPoint, SourcePoint


OCCUPIED = 100
FREE = 0
DEFAULT_LAYOUT_PATH = Path(__file__).resolve().parents[1] / "data" / "map_layout.yaml"
_DISK_OFFSETS_CACHE: dict[int, tuple[tuple[int, int], ...]] = {}


def _disk_offsets(radius_cells: int) -> tuple[tuple[int, int], ...]:
    cached = _DISK_OFFSETS_CACHE.get(radius_cells)
    ...
    _DISK_OFFSETS_CACHE[radius_cells] = offsets
    return offsets
...
class GridOccupancyMap:
    ...
    static_true_map: np.ndarray | None = None
    static_planning_map: np.ndarray | None = None
    _last_sync_signature: tuple[object, ...] | None = None
    ...
    def from_layout(
        ...
        occupancy.rebuild()
        occupancy._last_sync_signature = occupancy._semantic_sync_signature(
            occupancy.active_start_ids,
            occupancy.active_match_ids,
            occupancy.dynamic_circles,
        )
        return occupancy
    ...
    def clone(self) -> "GridOccupancyMap":
        cloned = GridOccupancyMap(
            ...
            static_true_map=None if self.static_true_map is None else self.static_true_map.copy(),
            static_planning_map=None if self.static_planning_map is None else self.static_planning_map.copy(),
            _last_sync_signature=self._last_sync_signature,
        )
        return cloned

    @staticmethod
    def _semantic_sync_signature(
        active_start_ids: set[str],
        active_match_ids: set[str],
        dynamic_circles: list[tuple[float, float, float]],
    ) -> tuple[object, ...]:
        return (
            frozenset(active_start_ids),
            frozenset(active_match_ids),
            tuple(dynamic_circles),
        )
    ...
    def sync_semantic_state(
        ...
        new_dynamic_circles = self.dynamic_circles if dynamic_circles is None else list(dynamic_circles)
        new_signature = self._semantic_sync_signature(
            active_start_ids,
            active_match_ids,
            new_dynamic_circles,
        )
        if new_signature == self._last_sync_signature:
            return
        self.active_start_ids = active_start_ids
        self.active_match_ids = active_match_ids
        self.dynamic_circles = new_dynamic_circles
        self.rebuild()
        self._last_sync_signature = new_signature

    def rebuild(self) -> None:
        if self.static_true_map is None or self.static_planning_map is None:
            self._rebuild_static_maps()

        assert self.static_true_map is not None
        assert self.static_planning_map is not None

        raw = self.static_true_map.copy()
        planning = self.static_planning_map.copy()
        dynamic_mask = np.zeros(self.shape, dtype=bool)
        ...
        for obstacle_id in self.active_start_ids:
            ...
            if obstacle is not None:
                ...
                self._add_rectangle(dynamic_mask, obstacle, True)
        ...
        for cx, cy, radius_m in self.dynamic_circles:
            ...
            self._add_circle(dynamic_mask, cx, cy, radius_m, True)

        obstacle_radius = int(round(self.config.obstacle_inflation_m / self.config.resolution_m))
        if dynamic_mask.any():
            planning = self._dilate_mask(planning, dynamic_mask, obstacle_radius)

        self.true_map = raw
        self.planning_map = planning

    def _rebuild_static_maps(self) -> None:
        ...
        self.static_true_map = raw
        self.static_planning_map = self._inflate_binary(raw)
    ...
    def _inflate_binary(self, raw: np.ndarray) -> np.ndarray:
        ...
        obstacle_mask = (raw == OCCUPIED) & ~border_mask
    ...
    def _dilate_mask(raw: np.ndarray, mask: np.ndarray, radius_cells: int) -> np.ndarray:
        ...
        for dr, dc in _disk_offsets(radius_cells):
            ...
            src_row_end = height - max(0, dr)
            src_col_start = max(0, -dc)
            src_col_end = width - max(0, dc)
            dst_row_start = max(0, dr)
            dst_row_end = height - max(0, -dr)
            dst_col_start = max(0, dc)
            dst_col_end = width - max(0, -dc)
            inflated_mask[dst_row_start:dst_row_end, dst_col_start:dst_col_end] |= mask[
                src_row_start:src_row_end,
                src_col_start:src_col_end,
            ]
        result[inflated_mask] = OCCUPIED
        return result

...
class RLInfraTests(unittest.TestCase):
    ...
    def test_play_match_worker_reuses_models_and_planner_within_process(self) -> None:
        ...
        with mock.patch("poc.rl_selfplay._WORKER_RUNTIME", None), \
             mock.patch("poc.rl_selfplay.torch", SimpleNamespace(
                 set_num_threads=lambda _threads: None,
                 set_num_interop_threads=lambda _threads: None,
                 manual_seed=lambda _seed: None,
                 device=lambda name: name,
             )), \
             mock.patch("poc.rl_selfplay.UtilityPlanner") as planner_cls, \
             mock.patch("poc.rl_selfplay.build_model", side_effect=[FakeModel("learner"), FakeModel("opponent")]) as build_model_mock, \
             mock.patch("poc.rl_selfplay.load_compatible_state_dict") as load_state_mock, \
             mock.patch("poc.rl_selfplay.TorchPolicySelector", side_effect=FakeSelector), \
             mock.patch("poc.rl_selfplay.play_match", return_value=fake_artifacts) as play_match_mock:
    ...
    def test_persistent_worker_pools_reuse_executors_and_close_them(self) -> None:
        ...
        with mock.patch("poc.rl_selfplay._make_process_pool", side_effect=build_fake_pool):
...
class TorchMaskTests(unittest.TestCase):
    def test_rng_state_normalization_accepts_legacy_serialized_formats(self) -> None:
        ...
        from poc.rl_checkpoint import normalize_torch_cuda_rng_state_all, normalize_torch_rng_state
    ...
    def test_masked_logits_block_invalid_actions(self) -> None:
        ...
        from poc.rl_checkpoint import load_checkpoint, save_policy_checkpoint
        from poc.rl_model import MaskedPolicyValueNet, greedy_action, load_compatible_state_dict
        from poc.rl_selfplay import action_dim, build_model, observation_dim
